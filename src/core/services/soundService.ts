...
class SoundService {
  ...
  // Miles Morales Bio-Electric Venom Blast Sound
  playVenomBlastSound() {
  ...
  // Wolverine Adamantium Claw Snikt Slash Sound
  playClawSlashSound() {
    this.triggerHaptic([10, 20, 30]);
    const ctx = this.getAudioContext();
    if (!ctx) return;
  ...
  // Ghost-Spider Gwen Stacy Portal Sound
  playGwenPortalSound() {
  ...
  // Captain America Vibranium Shield Sound
  playShieldClackSound() {
  ...
  // Hulk Smash Ground Pound Sound
  playHulkSmashSound() {
    this.triggerHaptic([60, 100, 140]);
    const ctx = this.getAudioContext();
    if (!ctx) return;
  ...
  // Batman Batarang Swish Sound
  playBatmanBatarangSound() {
  ...
  // Superman Heat Vision Beam Sound
  playSupermanLaserSound() {
    this.triggerHaptic([20, 40, 60]);
    const ctx = this.getAudioContext();
    if (!ctx) return;
  ...
  // Flash Speedforce Lightning Whoosh
  playFlashSpeedSound() {
    this.triggerHaptic([10, 20, 30, 40]);
    const ctx = this.getAudioContext();
    if (!ctx) return;
